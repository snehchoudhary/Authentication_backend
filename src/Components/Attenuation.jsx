...
function Attenuation() {
  ...
  useEffect(() => {
    ...
    const hasAttenuation = fileDataXLI.some(item =>
      Object.keys(item).some(key => key.trim().toLowerCase() === 'attenuation'));

    if (!hasVirtualDistance || !hasAttenuation) {
      console.error('Required columns "VirtualDistance (m)" or "Attenuation" not found in data');
      return;
    }
    ...
    const filteredData = fileDataXLI.filter(
      (item) => {
        ...
        const attenuationKey = Object.keys(item).find(key => key.trim().toLowerCase() === 'attenuation');
        return (
          item[vdKey] !== undefined &&
          item[vdKey] !== null &&
          item[vdKey].toString().trim() !== '' &&
          !isNaN(Number(item[vdKey])) &&
          item[attenuationKey] !== undefined &&
          item[attenuationKey] !== null &&
          item[attenuationKey].toString().trim() !== '' &&
          !isNaN(Number(item[attenuationKey]))
        );
      }
    );
    ...
    const dataOn = filteredData.map((item) => {
      const attenuationKey = Object.keys(item).find(key => key.trim().toLowerCase() === 'attenuation');
      return Number(item[attenuationKey]);
    });
  ...
  useEffect(() => {
    setChartOptions({
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'Virtual Distance vs Attenuation',
        },
  ...
  //     item['VirtualDistance (m)'] !== undefined && item['Attenuation'] !== undefined &&
  //     !isNaN(Number(item['VirtualDistance (m)'])) && !isNaN(Number(item['Attenuation']))
  //   );
      ...
      <div style={{ marginTop: '2rem', marginBottom: '1rem' }}>
        <button onClick={() => {
          ...
          if (input && !isNaN(input)) {
            window.open(`/attenuation-chunks?chunk=${input}`, '_blank');
          }
        }}>
